...
namespace AzureFunctions
{
    ...
    {
        // TEST URL : http://localhost:7000/api/SearchTour
        [FunctionName("SearchTour")]
            ...
            ILogger log)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(connectionString))
                {
                    ...
                    var query = "SELECT * FROM Tour WHERE TourCost >= '" + decimal.Parse(cost) + "' ORDER BY TourDestination ASC ";
                    ...
                    var toursResult = new DataTable();
                    toursResult.Load(sqlDataReader);
                    //Sending data in JSON form
                    message = JsonConvert.SerializeObject(toursResult);
                }
            }
            catch (Exception ex)
            {
